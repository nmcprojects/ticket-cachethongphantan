...
export const Event = () => {
  ...
  const eventList = useAppSelector(state => state.gateway.event.entities);
  const loading = useAppSelector(state => state.gateway.event.loading);
  const totalItems = useAppSelector(state => state.gateway.event.totalItems);
  ...
  return (
    <div>
      <h2 id="event-heading" data-cy="EventHeading">
        <Translate contentKey="gatewayApp.eventServiceEvent.home.title">Events</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="gatewayApp.eventServiceEvent.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/event/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="gatewayApp.eventServiceEvent.home.createLabel">Create new Event</Translate>
          </Link>
        </div>
      </h2>
        ...
        {eventList && eventList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="gatewayApp.eventServiceEvent.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('bannerUrl')}>
                  <Translate contentKey="gatewayApp.eventServiceEvent.bannerUrl">Banner Url</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('bannerUrl')} />
                </th>
                ...
                <th>
                  <Translate contentKey="gatewayApp.eventServiceEvent.organizer">Organizer</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {eventList.map((event, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`gatewayApp.EventStatus.${event.status}`} />
                  </td>
                  <td>{event.createdAt ? <TextFormat type="date" value={event.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{event.updatedAt ? <TextFormat type="date" value={event.updatedAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{event.organizer ? <Link to={`/organizer-profile/${event.organizer.id}`}>{event.organizer.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gatewayApp.eventServiceEvent.home.notFound">No Events found</Translate>
            </div>
          )
        )}
      </div>
