  ...
  return (
    <div>
      <h2 id="payment-heading" data-cy="PaymentHeading">
        <Translate contentKey="gatewayApp.paymentServicePayment.home.title">Payments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="gatewayApp.paymentServicePayment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/payment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="gatewayApp.paymentServicePayment.home.createLabel">Create new Payment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {paymentList && paymentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="gatewayApp.paymentServicePayment.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('failedAt')}>
                  <Translate contentKey="gatewayApp.paymentServicePayment.failedAt">Failed At</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('failedAt')} />
                </th>
            ...
            <tbody>
              {paymentList.map((payment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`gatewayApp.PaymentStatus.${payment.status}`} />
                  </td>
                  <td>{payment.paidAt ? <TextFormat type="date" value={payment.paidAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{payment.failedAt ? <TextFormat type="date" value={payment.failedAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{payment.expiredAt ? <TextFormat type="date" value={payment.expiredAt} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gatewayApp.paymentServicePayment.home.notFound">No Payments found</Translate>
            </div>
          )
        )}
      </div>
